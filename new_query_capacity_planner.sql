--  new Redash query capacity planner #54514

with c as (
select 
date(dt) as day
,dyw as weekday
from wkda_retail_purchase_fr.ref_calendar as cal
...
from wkda_retail_purchase_fr.retail_handover_appointment_shift as sh
left join wkda_retail_purchase_fr.branches as b on b.id = sh.branch_id
left join wkda_retail_purchase_fr.aux_user as aux on aux.id = sh.user_id
...
from wkda_retail_purchase_fr.retail_handover_appointment_shift as sh
left join wkda_retail_purchase_fr.branches as b on b.id = sh.branch_id
left join wkda_retail_purchase_fr.retail_handover_appointment_blocked_time as blo on blo.shift_id = sh.id
left join wkda_retail_purchase_fr.aux_user as aux on aux.id = sh.user_id
